...
class Raider:
    ...
    def joiner(self, token, invite):
        try:
            ...
            match response.status_code:
                case 200:
                    console.log(f"JOINED", C["green"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                f"{response.json()['guild']['name']}")
                case 400:
                    console.log("CAPTCHA", C["yellow"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                f"discord.gg/{invite}")
                case 429:
                    console.log("CLOUDFARE", C["magenta"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                f"discord.gg/{invite}")
                case _:
            ...
            while True:
                ...
                match response.status_code:
                    ...
                    case 429:
                        ...
                        console.log("RATELIMIT", Fore.LIGHTYELLOW_EX, f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                    f"Ratelimit Exceeded - {retry_after}s", )
            ...
            try:
                while True:
                    ...
                    match response.status_code:
                        ...
                        case 429:
                            retry_after = response.json().get('retry_after')
                            console.log("RATELIMITED", C["pink"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                        f"{retry_after}s")
                            time.sleep(retry_after)
                        case _:
                            console.log("Invalid", C["red"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                        response.json().get("message"))
                            break
        ...
        try:
            ...
            def add_reaction(token):
                try:
                    ...
                    match response.status_code:
                        case 204:
                            console.log("Reacted", C["green"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                        selected)
                        case _:
                            console.log("Failed", C["red"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                        response.json().get("message"))
    ...
    def soundbord(self, token, channel):
        try:
            ...
            while True:
                ...
                match response.status_code:
                    case 204:
                        console.log(C["light_blue"], f"Successfully played sound {Fore.YELLOW}{name}",
        ...
        def main_checker(token):
            try:
                while True:
                    ...
                    match response.status_code:
                        case 200:
                            console.log("Found", C["green"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                        guild_id)
                            in_guild.append(token)
                            break
                        case 429:
                            retry_after = response.json().get("retry_after")
                            console.log("RATELIMIT", Fore.LIGHTYELLOW_EX,
                                        f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                        f"Ratelimit Exceeded - {retry_after}s", )
                            time.sleep(float(retry_after))
                        case _:
                            console.log("Not Found", C["red"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                        guild_id)
                            break
                with open("data/tokens.txt", "w") as f:
                    f.write("\n".join(in_guild))
    ...
    def friender(self, token, nickname):
        try:
            ...
            match response.status_code:
                case 204:
                    console.log(f"SUCCESS -> added {nickname}", C["green"],
                                f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**")
                case 400:
                    console.log("CAPTCHA", C["yellow"], f"{Fore.RESET}{token[:25]}.{Fore.LIGHTCYAN_EX}**",
                                response.json())
...
class Menu:
    ...
    def joiner(self):
        ...
        if invite == "":
            Menu().main_menu()
        invite = invite.replace("https://discord.gg/", "").replace("https://discord.com/invite/", "").replace(
            "discord.gg/", "").replace("https://discord.com/invite/", "").replace(".gg/", "").replace("https://canary.",
                                                                                                      "").replace(
            "canary.", "")
        os.system('cls')
        console.render_ascii()
